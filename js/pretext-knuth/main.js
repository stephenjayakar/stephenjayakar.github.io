(() => {
  ...
  // ns-hugo:/Users/stephen/programming/stephen-blog/assets/js/pretext-knuth/dom-map.js
  var SKIP_SELECTOR = "pre, figure, table, script, style, .katex, mjx-container, .MathJax";
  ...
  function extractDomText(root) {
    ...
    function walk(node) {
      if (node.nodeType === Node.TEXT_NODE) {
        ...
        for (let i = 0; i < data.length; i++) {
          charMap.push({ textNode, charIndex: i });
          plainText += data[i];
        }
        return;
      }
      ...
      if (tag === "BR") {
        charMap.push({ textNode: null, charIndex: -1, literal: "\n" });
        plainText += "\n";
        return;
      }
      if (tag === "IMG" || tag === "FIGURE" || tag === "VIDEO" || tag === "PICTURE") {
        return;
      }
      if (el !== root && BLOCK_TAGS.has(tag)) {
    ...
    return { plainText, charMap };
  }

  // ns-hugo:/Users/stephen/programming/stephen-blog/assets/js/pretext-knuth/hyphenate.js
  var SOFT_HYPHEN = "\xAD";
  ...
  function rangeHasProtectedMarkup(charMap, start, end) {
    const protectedTags = /* @__PURE__ */ new Set(["A", "CODE"]);
    for (let i = start; i < end; i++) {
      const entry = charMap[i];
      if (!(entry == null ? void 0 : entry.textNode)) continue;
      let el = entry.textNode.parentElement;
      while (el) {
        if (protectedTags.has(el.tagName)) return true;
        el = el.parentElement;
      }
    }
    return false;
  }
  function buildPlainToHyphenMap(plainText, hyphenated) {
    const plainToHyphen = new Array(plainText.length);
    let pi = 0;
    let hi = 0;
    while (pi < plainText.length) {
      while (hi < hyphenated.length && hyphenated[hi] === SOFT_HYPHEN) {
        hi++;
      }
      plainToHyphen[pi] = hi < hyphenated.length ? hi : hyphenated.length;
      pi++;
      if (hi < hyphenated.length) {
        hi++;
      }
    }
    return plainToHyphen;
  }
  function hyphenatePlainText(plainText, charMap = []) {
    let hyphenated = "";
    let searchFrom = 0;
    ...
    for (let index = 0; index < parts.length; index++) {
      ...
      const tokenStart = plainText.indexOf(token, searchFrom);
      const tokenEnd = tokenStart + token.length;
      searchFrom = tokenEnd;
      const allowHyphen = charMap.length === 0 || !rangeHasProtectedMarkup(charMap, tokenStart, tokenEnd);
      const wordParts = allowHyphen ? hyphenateWord(token) : [token];
      hyphenated += wordParts.length <= 1 ? token : wordParts.join(SOFT_HYPHEN);
    }
    return {
      hyphenated,
      plainToHyphen: buildPlainToHyphenMap(plainText, hyphenated)
    };
  }
  ...
  // ns-hugo:/Users/stephen/programming/stephen-blog/assets/js/pretext-knuth/knuth-layout.js
  ...
  function createLayoutResources(font) {
    const measureCanvas = document.createElement("canvas");
    const measureCtx = measureCanvas.getContext("2d");
    if (measureCtx === null) {
      throw new Error("2D canvas context is required for Pretext Knuth layout");
    }
  ...
  function layoutParagraphOptimal(prepared, maxWidth, resources) {
    ...
    for (let toCandidate = 1; toCandidate < candidateCount; toCandidate++) {
      ...
      for (let fromCandidate2 = toCandidate - 1; fromCandidate2 >= 0; fromCandidate2--) {
        if (dp[fromCandidate2] === Infinity) continue;
    ...
    for (let index = 0; index < breakIndices.length; index++) {
      ...
      const fromSegIndex = breakCandidates[fromCandidate].segIndex;
      const toSegIndex = breakCandidates[toCandidate].segIndex;
      lines.push(
  ...
  function layoutHyphenatedText(hyphenatedText, font, maxWidth, resources) {
    const prepared = prepareWithSegments(hyphenatedText, font);
    const lines = layoutParagraphOptimal(prepared, maxWidth, resources);
  ...
  function lineBadness(lineStats, maxWidth, normalSpaceWidth, isLastLine) {
    ...
    const badness = absRatio * absRatio * absRatio * 1e3;
    const riverExcess = justifiedSpace / normalSpaceWidth - RIVER_THRESHOLD;
    const riverPenalty = riverExcess > 0 ? 5e3 + riverExcess * riverExcess * 1e4 : 0;
    const tightThreshold = normalSpaceWidth * TIGHT_SPACE_RATIO;
    const tightPenalty = justifiedSpace < tightThreshold ? 3e3 + (tightThreshold - justifiedSpace) * (tightThreshold - justifiedSpace) * 1e4 : 0;
    const hyphenPenalty = lineStats.trailingMarker === "soft-hyphen" ? 50 : 0;
    return badness + riverPenalty + tightPenalty + hyphenPenalty;
  }
  function buildMeasuredLineFromCandidateRange(prepared, breakCandidates, fromCandidate, toCandidate, maxWidth, hyphenWidth, fromSegIndex, toSegIndex) {
    const from = fromSegIndex;
    const to = toSegIndex;
    ...
    for (let segIndex = from; segIndex < to; segIndex++) {
      ...
      if (text === SOFT_HYPHEN) continue;
      segments.push(toLineSegment(text, prepared.widths[segIndex], segIndex));
    }
    if (trailingMarker === "soft-hyphen" && ending === "wrap") {
      segments.push({ kind: "text", text: "-", width: hyphenWidth, sourceSegIndex: to - 1 });
    }
    trimTrailingSpaces(segments);
    return finalizeMeasuredLine(
      segments,
      maxWidth,
      ending,
      trailingMarker,
  ...
  function toLineSegment(text, width, sourceSegIndex) {
    if (isSpaceText(text)) return { kind: "space", width, sourceSegIndex };
    return { kind: "text", text, width, sourceSegIndex };
  }
  ...
  function finalizeMeasuredLine(segments, maxWidth, ending, trailingMarker, fromSegIndex, toSegIndex) {
    let wordWidth = 0;
    let spaceCount = 0;
    let naturalWidth = 0;
  ...
  // ns-hugo:/Users/stephen/programming/stephen-blog/assets/js/pretext-knuth/dom-render.js
  function cloneInlineWrapper(textNode, blockRoot) {
    let el = textNode.parentElement;
    if (!el || el === blockRoot) {
      return document.createTextNode("");
    }
    const chain = [];
    while (el && el !== blockRoot) {
      chain.push(el);
      el = el.parentElement;
    }
    if (chain.length === 0) {
    ...
    return rootClone || document.createTextNode("");
  }
  function buildFragmentFromPlainRange(charMap, plainStart, plainEnd, blockRoot) {
    const frag = document.createDocumentFragment();
    if (plainStart >= plainEnd) return frag;
    let i = plainStart;
    while (i < plainEnd) {
      const entry = charMap[i];
      if (entry.literal !== void 0) {
        frag.appendChild(document.createTextNode(entry.literal));
        i++;
        continue;
      }
      if (!entry.textNode) {
      ...
      let j = i + 1;
      while (j < plainEnd) {
        const next = charMap[j];
        if (next.literal !== void 0 || next.textNode !== entry.textNode) break;
        j++;
      }
      const sliceStart = charMap[i].charIndex;
      const sliceEnd = charMap[j - 1].charIndex + 1;
      const text = entry.textNode.data.slice(sliceStart, sliceEnd);
      const wrapper = cloneInlineWrapper(entry.textNode, blockRoot);
  ...
  function materializeHyphenSegmentRange(prepared, fromSegIndex, toSegIndex) {
    let text = "";
    for (let segIndex = fromSegIndex; segIndex < toSegIndex; segIndex++) {
      const segment = prepared.segments[segIndex];
      if (segment === SOFT_HYPHEN) continue;
      text += segment;
    }
    return text;
  }
  function alignPlainLineRange(plainText, plainCursor, hyphenLineText) {
    let pi = plainCursor;
    let hi = 0;
    while (hi < hyphenLineText.length && pi < plainText.length) {
      const hc = hyphenLineText[hi];
  ...
  function renderKnuthBlock(lines, prepared, plainText, charMap, blockRoot, normalSpaceWidth) {
    const block = document.createElement("div");
    block.className = "pretext-knuth-block";
    let plainCursor = 0;
    for (const line of lines) {
      const lineEl = document.createElement("div");
      const spacing = getDisplaySpacing(line, normalSpaceWidth);
      lineEl.className = spacing.kind === "justified" ? "pretext-knuth-line pretext-knuth-line--justified" : "pretext-knuth-line pretext-knuth-line--ragged";
      const hyphenLineText = materializeHyphenSegmentRange(
        prepared,
        line.fromSegIndex,
        line.toSegIndex
      );
      const { plainStart, plainEnd, nextCursor } = alignPlainLineRange(
        plainText,
        plainCursor,
        hyphenLineText
      );
      lineEl.appendChild(
        buildFragmentFromPlainRange(charMap, plainStart, plainEnd, blockRoot)
      );
      plainCursor = nextCursor;
  ...
  // <stdin>
  var BLOCK_SELECTOR = ".post-content > p, .post-content blockquote > p";
  var FONT_FAMILY = 'Georgia, "Times New Roman", serif';
  var KNUTH_STYLES = `
.post-content.pretext-knuth-active {
  font-family: Georgia, "Times New Roman", serif;
}
.post-content.pretext-knuth-active.pretext-knuth--loading {
  opacity: 0.92;
}
.pretext-knuth-source { margin: 0; }
.pretext-knuth-block { margin: 0; }
.pretext-knuth-line {
  width: 100%;
  line-height: 1.675;
  margin: 0;
}
.pretext-knuth-line--justified {
  display: block;
  text-align: justify;
  text-justify: inter-word;
}
.pretext-knuth-line--ragged {
  display: block;
}
.pretext-knuth-space {
...
.pretext-knuth-word { flex-shrink: 0; white-space: pre; }
.pretext-knuth-block + .pretext-knuth-block { margin-top: 0.6em; }
.post-content.pretext-knuth-active > p { margin: 0; }
.post-content.pretext-knuth-active > p + p { margin-top: 1.3rem; }
.pretext-knuth-active a { color: var(--primary-alt); }
.pretext-knuth-active a:hover { color: var(--primary); }
`;
  function ensureKnuthStyles() {
    if (document.getElementById("pretext-knuth-styles")) return;
    const style = document.createElement("style");
    style.id = "pretext-knuth-styles";
    style.textContent = KNUTH_STYLES;
    document.head.appendChild(style);
  }
  var layoutResources = null;
  var layoutFont = null;
  function isKnuthMode() {
    return new URLSearchParams(window.location.search).get("pretext") === "knuth";
  }
  ...
  function getPretextFont() {
    const rootPx = parseFloat(getComputedStyle(document.documentElement).fontSize);
    const sizePx = Math.round(rootPx * 1.125);
    return `${sizePx}px ${FONT_FAMILY}`;
  }
  function ensureResources(resources, font) {
    if (layoutResources && layoutFont === font) return layoutResources;
    layoutResources = resources;
    layoutFont = font;
    return layoutResources;
  }
  function layoutBlock(element, maxWidth, resources, font) {
    if (!element.dataset.pretextOriginal) {
      element.dataset.pretextOriginal = element.innerHTML;
    } else {
      element.innerHTML = element.dataset.pretextOriginal;
    }
    const { plainText, charMap } = extractDomText(element);
    if (!plainText.trim()) return;
    const { hyphenated } = hyphenatePlainText(plainText, charMap);
    const { prepared, lines } = layoutHyphenatedText(hyphenated, font, maxWidth, resources);
    const block = renderKnuthBlock(
      lines,
      prepared,
      plainText,
      charMap,
    ...
    element.innerHTML = "";
    element.classList.add("pretext-knuth-source");
    element.appendChild(block);
  }
  function layoutPostContent(postContent) {
    const font = getPretextFont();
    const resources = ensureResources(createLayoutResources(font), font);
    const blocks = postContent.querySelectorAll(BLOCK_SELECTOR);
    for (const block of blocks) {
      if (!(block instanceof HTMLElement)) continue;
      if (shouldSkipBlock(block)) continue;
      const maxWidth = getBlockWidth(block);
      if (maxWidth <= 0) continue;
      layoutBlock(block, maxWidth, resources, font);
    }
    postContent.classList.remove("pretext-knuth--loading");
  }
  ...
  function boot() {
    if (!isKnuthMode()) return;
    document.documentElement.dataset.pretext = "knuth";
    ensureKnuthStyles();
    const postContent = document.querySelector(".post-content");
    if (!(postContent instanceof HTMLElement)) return;
    postContent.classList.add("pretext-knuth-active", "pretext-knuth--loading");
    const run = () => layoutPostContent(postContent);
    run();
    const onResize = debounce(run, 150);
